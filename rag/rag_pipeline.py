...
class AutonomousLearningFilter:
    ...
    def _run(self, question: str, answer: str) -> None:
        ...
        if not self._llm.is_available():
            log.warning("AutoFilter: Qwen client unavailable — skipping critic.")
            return
        ...
        verdict = (self._llm.generate_raw(critic_prompt, temperature=0.0) or "").upper()
        if not verdict:
...
class RAGPipeline:
    ...
    def run(
        ...
        if memory is not None and memory.stage not in (LoanStage.COMPLETE, LoanStage.PENDING):
            next_stage = self._state_machine.next_stage(memory.stage, memory.entities)
            followup = _STAGE_FOLLOWUP_QUESTIONS.get(next_stage)
            if followup is not None:
                memory.stage = next_stage
                return RAGResponse(answer=followup, source=f"state_machine_{next_stage.value}_fallback")
